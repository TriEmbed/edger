...
const char *reset_key = "reset_count";
const char *ssid_key = "ssid";
const char *password_key = "password";
const char *timeout_key = "timeout";
const char *mdns = "mdns";
const char *i2c_clock = "i2c_clock";
const char *i2c_data = "i2c_data";
...
static const type_str_pair_t type_str_pair[] = {{NVS_TYPE_I8,   "i8"},
                                                {NVS_TYPE_U8,   "u8"},
                                                {NVS_TYPE_U16,  "u16"},
                                                {NVS_TYPE_I16,  "i16"},
                                                {NVS_TYPE_U32,  "u32"},
...
const char current_namespace[16] = "storage";
...
esp_err_t get_nvs_marker(int32_t *value) {
    ...
    // nvs_set_str (nvs, menu, menuJson);

    if (err != ESP_OK) {
        return err;
    }

    err = nvs_get_i32(nvs, reset_key, value);
    if (err != ESP_OK) {
        return err;
    }
    return ESP_OK;
...
esp_err_t set_nvs_marker(int32_t value) {
    ...
    err = nvs_set_i32(nvs, reset_key, value);
    if (err != ESP_OK) {
        return err;
    }
    return ESP_OK;
...
//    }
//
//  /// 0x5a5a5a5a will force a reset on the config
//  err = nvs_set_i32 (nvs, reset_key, value);
//    if ((err != ESP_OK) || (inc == 0x5a5a5a5a)) {
//        union {
...
//
//        char buf[32];
//        sprintf(buf, "esp-%x", (uint32_t) (device.value & 0xffffff));
//
//        err |= nvs_set_str(nvs, ssid, "dlink-018E");
//        err |= nvs_set_str(nvs, password, "qwertyui");
//        err |= nvs_set_str(nvs, mdns, buf);
//        err |= nvs_set_u8(nvs, i2c_clock, 0);
//        err |= nvs_set_u8(nvs, i2c_data, 1);
//    }
//
//    if (err != ESP_OK) {
//        return 0xfffff2;
...
//        err = nvs_commit(nvs);
//        if (err == ESP_OK) {
//            ESP_LOGI (TAG, "Value stored under key '%s' %x", reset_key, value);
//        }
//    }
//
//
...
// TODO PJS: get ssid and password array sizes for len assignments

void get_wifi(uint8_t *ssid, uint8_t *password, int64_t *timeout) {
    nvs_handle_t nvs;
    printf("in call addr(ssid) %x\n", ssid);
    printf("in call addr(password) %x\n", password);

    nvs_open(current_namespace, NVS_READWRITE, &nvs);
    size_t len = 64;
    nvs_get_str(nvs, password_key,(char *) password, &len);
    len = 32;
    nvs_get_str(nvs, ssid_key, (char *) ssid, &len);
    nvs_get_i64(nvs, timeout_key, timeout);
    nvs_close(nvs);
}
...
/* set default value*/
    ...
    nvs_set_str(nvs, ssid_key, ssid);
    nvs_set_str(nvs, password_key, password);
    nvs_set_i64(nvs, timeout_key, timeout);
    nvs_close(nvs);
}
...
cJSON *preset_nvs() {
    ...
    size_t len;
    if (nvs_get_str(nvs, ssid_key, NULL, &len) == ESP_OK) {

        if (len > 1) {

            char *str = (char *) malloc(len);
            if (nvs_get_str(nvs, ssid_key, str, &len) == ESP_OK) {
                cJSON_AddStringToObject(root, ssid_key, str);
            }
            free(str);
        } else {

            nvs_set_str(nvs, ssid_key, CONFIG_EXAMPLE_WIFI_SSID);
            cJSON_AddStringToObject(root, ssid_key, CONFIG_EXAMPLE_WIFI_SSID);
    ...
    if (nvs_get_str(nvs, mdns, NULL, &len) == ESP_OK) {
        char *str = (char *) malloc(len);
        if (nvs_get_str(nvs, mdns, str, &len) == ESP_OK) {
            cJSON_AddStringToObject(root, mdns, str);
        }
        free(str);
    }

    if (nvs_get_str(nvs, password_key, NULL, &len) == ESP_OK) {
        if (len > 1) {
            char *str = (char *) malloc(len);
            if (nvs_get_str(nvs, password_key, str, &len) == ESP_OK) {
                cJSON_AddStringToObject(root, password_key, str);
            }
            free(str);
        } else {
            cJSON_AddStringToObject(root, password_key, CONFIG_EXAMPLE_WIFI_PASSWORD);
            nvs_set_str(nvs, password_key, CONFIG_EXAMPLE_WIFI_PASSWORD);
    ...
    if (nvs_get_i32(nvs, timeout_key, &value) == ESP_OK) {
        cJSON_AddNumberToObject(root, timeout_key, value);
    }

    if (nvs_get_u8(nvs, i2c_clock, &k) == ESP_OK) {

        cJSON_AddNumberToObject(root, i2c_clock, k);
    }

    if (nvs_get_u8(nvs, i2c_data, &k) == ESP_OK) {

        cJSON_AddNumberToObject(root, i2c_data, k);
    }
...
esp_err_t set_value_in_nvs(const char *key, const char *str_type, const char *str_value) {
    esp_err_t err;
    nvs_handle_t nvs;
...
static int list(const char *part, __attribute__((unused)) const char *name, const char *str_type) {
    nvs_type_t type = str_to_type(str_type);
...
int set_namespace(int argc, char **argv) {
...
void register_nvs(void) {
    ...
    const esp_console_cmd_t set_cmd = {.command = "nvs_set", .help = "Set key-value pair in selected namespace.\n"
                                                                     "Examples:\n"
                                                                     " nvs_set VarName i32 -v 123 \n"
                                                                     " nvs_set VarName str -v YourString \n"
                                                                     " nvs_set VarName blob -v 0123456789abcdef \n", .hint = NULL, .func = &set_value, .argtable = &set_args};
    const esp_console_cmd_t get_cmd = {.command = "nvs_get", .help = "Get key-value pair from selected namespace. \n"
                                                                     "Example: nvs_get VarName i32", .hint = NULL, .func = &get_value, .argtable = &get_args};
    const esp_console_cmd_t erase_cmd = {.command = "nvs_erase", .help = "Erase key-value pair from current namespace", .hint = NULL, .func = &erase_value, .argtable = &erase_args};
    const esp_console_cmd_t erase_namespace_cmd = {.command = "nvs_erase_namespace", .help = "Erases specified namespace", .hint = NULL, .func = &erase_namespace, .argtable = &erase_all_args};
    const esp_console_cmd_t namespace_cmd = {.command = "nvs_namespace", .help = "Set current namespace", .hint = NULL, .func = &set_namespace, .argtable = &namespace_args};
    const esp_console_cmd_t list_entries_cmd = {.command = "nvs_list", .help = "List stored key-value pairs stored in NVS."
                                                                               "Namespace and type can be specified to print only those key-value pairs.\n"
                                                                               "Following command list variables stored inside 'nvs' partition, under namespace 'storage' with type uint32_t"
                                                                               "Example: nvs_list nvs -n storage -t u32 \n", .hint = NULL, .func = &list_entries, .argtable = &list_args};

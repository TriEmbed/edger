...
static const char *menuJson = "{\"path\": \"/api/system/v1/write\","
  "\"text\": \"Renesas\","
  "\"icon\": \"R\","
  "\"hidden\": false,"
  "\"to\": \"/api/system/v1/write\","
  "\"permissions\": [],"
  "\"type\": \"MENU\","
  "\"resource\": \"Reneses\","
  "\"children\": ["
  "{\"path\": \"/api/system/v1/renesas/read\","
  "\"text\": \"Read File\","
  "\"icon\": \"F\","
  "\"hidden\": false,"
  "\"to\": \"/api/system/v1/renesas/read\","
  "\"permissions\": [],"
  "\"type\": \"VIEW\","
  "\"resource\": \"Renesas/file/readFile\"},"
  "{\"path\": \"/api/system/v1/rensas/other\","
  "\"text\": \"Other\","
  "\"icon\": \"O\","
  "\"hidden\": false,"
  "\"to\": \"/api/system/v1/rensas/other\","
  "\"permissions\": [],"
  "\"type\": \"VIEW\", \"resource\": \"Renesas/other\"}]}";
...
static esp_err_t
store_blob (nvs_handle_t nvs, const char *key, const char *str_values)
{
  uint8_t value;
...
static void
print_blob (const char *blob, size_t len)
{
  for (int i = 0; i < len; i++)
    {
      printf ("%02x", blob[i]);
...
preset_nvs ()
{
  ...
  if ((err = nvs_get_str (nvs, ssid, NULL, &len)) == ESP_OK)
    {
      ...
      if ((err = nvs_get_str (nvs, ssid, str, &len)) == ESP_OK)
	{
  ...
  if ((err = nvs_get_str (nvs, mdns, NULL, &len)) == ESP_OK)
    {
      char *str = (char *) malloc (len);
      if ((err = nvs_get_str (nvs, mdns, str, &len)) == ESP_OK)
	{
  ...
  if ((err = nvs_get_str (nvs, password, NULL, &len)) == ESP_OK)
    {
      char *str = (char *) malloc (len);
      if ((err = nvs_get_str (nvs, password, str, &len)) == ESP_OK)
	{
  ...
  if ((err = nvs_get_i32 (nvs, reset_key, &value)) == ESP_OK)
    {
      cJSON_AddNumberToObject (root, reset_key, value);
    }

  if ((err = nvs_get_u8 (nvs, i2c_clock, &k)) == ESP_OK)
    {

      cJSON_AddNumberToObject (root, i2c_clock, k);
    }

  if ((err = nvs_get_u8 (nvs, i2c_data, &k)) == ESP_OK)
    {
...
esp_err_t
set_value_in_nvs (const char *key, const char *str_type,
...
static esp_err_t
get_value_from_nvs (const char *key, const char *str_type)
{
  nvs_handle_t nvs;
  esp_err_t err;
  nvs_type_t type = str_to_type (str_type);
...
static int
list (const char *part, const char *name, const char *str_type)
{
  nvs_type_t type = str_to_type (str_type);
...
static int
erase_value (int argc, char **argv)
{
...
static int
erase_namespace (int argc, char **argv)
{
...
int
set_namespace (int argc, char **argv)
{
...
static int
list_entries (int argc, char **argv)
{
...
register_nvs (void)
{
  ...
  get_args.key =
    arg_str1 (NULL, NULL, "<key>", "key of the value to be read");
  ...
  namespace_args.namespace =
    arg_str1 (NULL, NULL, "<namespace>",
	      "namespace of the partition to be selected");
  namespace_args.end = arg_end (2);
  list_args.partition =
    arg_str1 (NULL, NULL, "<partition>", "partition name");
  list_args.namespace =

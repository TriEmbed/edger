...
cJSON *i2cInfo() {
    ...
    cJSON_AddStringToObject(response, "mode", conf.mode == I2C_MODE_MASTER ? "MASTER" : "SLAVE");
    ...
    cJSON_AddNumberToObject(response, "master clk speed", conf.master.clk_speed);
...
cJSON *i2cSet(uint8_t chip_addr, uint8_t index, cJSON *data) {
